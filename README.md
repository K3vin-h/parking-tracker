...
For a detailed explanation of the architecture and each page, see
[`docs/day-9-10-frontend-dashboard-explained.md`](docs/day-9-10-frontend-dashboard-explained.md).

## Checking Day 9 and Day 10

Start the application and prepare the database:
...
Open `http://localhost:8000/login/`, sign in with a staff/superuser account, and
visit each page in the table above. `setup_defaults` creates the initial superuser
from `DEFAULT_SUPERUSER_EMAIL` and `DEFAULT_SUPERUSER_PASSWORD`. Alternatively,
...
Populate sessions and detection events through the upload page. Django admin can
create test sessions for dashboard and chart data, but it bypasses upload-specific
image validation and should not be used to verify upload security.

Real uploads require trained weights at
`apps/cv/weights/detector.pth` and `apps/cv/weights/recognizer.pth`, or the
equivalent configured environment paths. When weights are unavailable, the
upload endpoint intentionally returns HTTP 503.
...
Use browser developer tools to check the responsive layout at approximately
1440, 1024, 768, and 320 pixels wide. At mobile widths, verify that the sidebar
opens as a drawer, closes from the scrim or Escape key, and keeps the correct
