...
# In production the loopback fallback is unsafe: behind a same-host reverse proxy
# REMOTE_ADDR is 127.0.0.1 for EVERY public client, so without a token any client
# could reach the DB-touching /health/ probe. Require the token when DEBUG=False.
if not DEBUG and not HEALTH_CHECK_TOKEN:
    raise ImproperlyConfigured(
        "HEALTH_CHECK_TOKEN must be set when DEBUG=False. The /health/ probe runs "
