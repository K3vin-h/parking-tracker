...
wallet. Top-ups go through the placeholder payment gateway and only credit the
wallet after that gateway reports success, so swapping in a real provider changes
...
def topup(request):
    """
    Add funds to the current user's wallet via the placeholder payment gateway.

    The wallet is credited ONLY after the gateway confirms the charge succeeded.
    """
