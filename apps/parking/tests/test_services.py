...
class TestCalculateCharge:
    ...
    def test_non_positive_duration_is_free(self):
        ...
        assert calculate_charge(
            entry, entry - timedelta(minutes=5), _settings()
        ) == Decimal("0.00")
...
class TestHandleEntry:
    ...
    def test_guest_entry_creates_unlinked_session(self, parking_lot, lot_settings):
        ...
        session = handle_entry(
            "GUEST99", 0.9, [0.1, 0.2, 0.3, 0.4], PLATE_IMAGE, parking_lot
        )
    ...
    def test_registered_entry_links_user_and_plate(
    ...
    def test_low_confidence_flagged_but_session_created(
    ...
    def test_bounding_box_is_sanitized(self, parking_lot, lot_settings):
        ...
        session = handle_entry(
            "BBOX01", 0.9, [1.5, -0.2, 0.5, 0.5], PLATE_IMAGE, parking_lot
        )
        event = PlateDetectionEvent.objects.get(session=session)
        assert event.bounding_box == [1.0, 0.0, 0.5, 0.5]

        session2 = handle_entry(
            "BBOX02", 0.9, [0.1, 0.2, 0.3], PLATE_IMAGE, parking_lot
        )
...
class TestHandleExit:
    ...
    def test_normal_exit_completes_and_bills(self, parking_lot, lot_settings):
        ...
        assert PlateDetectionEvent.objects.filter(
            session=session, event_type="exit"
        ).exists()
...
class TestCorrectPlate:
    ...
    def test_entry_correction_voids_duplicate_active_session(
        self, parking_lot, lot_settings
    ):
    ...
    def test_correction_relinks_to_registered_plate(
        self, parking_lot, lot_settings, license_plate, user
    ):
    ...
    def test_correction_clears_link_for_unregistered(
        self, parking_lot, lot_settings, license_plate, user
    ):
    ...
    def test_orphan_exit_correction_ignores_later_reentry(
        self, parking_lot, lot_settings
    ):
    ...
    def test_orphan_exit_correction_does_not_revive_voided_reentry_session(
        self, parking_lot, lot_settings
    ):
        """Correcting a queued exit must NOT revive a session voided by re-entry.

        A session voided because the same plate re-entered was correctly
        abandoned; reviving and billing it here would double-bill the vehicle
        (the live re-entry session bills on its own exit). The correction finds
        no OPEN session to close and leaves the void untouched.
        """
        ...
        result.refresh_from_db()
        # The voided orphan stays void with no charge — not double-billed.
        assert original.status == "void"
        assert original.was_orphaned is True
        assert original.exit_time is None
        assert original.charge_amount == Decimal("0.00")
        # The live re-entry session is left active to bill on its own exit.
        assert reentry.status == "active"
        # No open session matched the corrected exit, so it stays unlinked.
        assert result.session is None
