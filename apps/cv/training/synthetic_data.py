...
# Minimum fraction of requested samples a generation run must yield.
# Below this the run aborts: a high skip rate means something is
# systematically broken (corrupt backgrounds, full disk), and silently
# producing a fraction of the dataset would undertrain the models.
...
def _load_font(size: int = _FONT_SIZE) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
    ...
    WHY logger.error (not warning): the lru_cache means this message fires
    exactly once per process, then every subsequent sample silently uses the
    degraded font.  In a 50k-sample run that single line is the only signal
    that the whole dataset was rendered at lower fidelity — it must stand out.
    """
    if _FONT_PATH.exists():
        try:
            ...
            logger.error(
                ...
                "falling back to PIL default — the ENTIRE dataset generated "
                "by this process will use the lower-fidelity font.",
                _FONT_PATH,
                exc,
            )
    else:
        logger.error(
            "Plate font not found — using PIL default (lower fidelity) for "
            "the ENTIRE dataset generated by this process. "
...
def generate_detector_dataset(
    ...
    if generated_count == 0 and last_error is not None:
        raise last_error
    # WHY a yield floor: raising only on a fully-empty dataset lets a run with
    # 90 % skipped samples "succeed" and silently train an undersized model.
    # A high skip rate means something is systematically broken (corrupt
    # backgrounds, full disk) — fail loudly instead of degrading quietly.
    if generated_count < n * _MIN_YIELD_FRACTION:
        raise RuntimeError(
            f"Detector dataset generation yielded only {generated_count} of "
            f"{n} requested samples ({skip_count} skipped). Aborting — "
...
def generate_recognizer_dataset(
    ...
    # Same yield floor as the detector variant: an empty or mostly-empty
    # labels.csv would otherwise surface much later as a confusing
    # "Dataset is empty" error inside the training script.
    if generated_count < n * _MIN_YIELD_FRACTION:
        raise RuntimeError(
            f"Recognizer dataset generation yielded only {generated_count} of "
            f"{n} requested samples ({skip_count} skipped). Aborting — "
            "inspect the warnings above for the root cause."
        )
